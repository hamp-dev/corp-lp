(() => {
  ...
  const imageTextByLanguage = {
    en: {
      fallbackAlt: "hamp interface screenshot",
      openLabel: (alt) => `Open ${alt}`,
      openPublicPage: "Open public page",
    },
    ko: {
      fallbackAlt: "hamp 조작 화면",
      openLabel: (alt) => `${alt} 열기`,
      openPublicPage: "공개 페이지 열기",
    },
    ja: {
      fallbackAlt: "hampの操作画面",
      openLabel: (alt) => `${alt}を開く`,
      openPublicPage: "公開ページを開く",
    },
  };
  const imageText = imageTextByLanguage[pageLanguage] || imageTextByLanguage.ja;
  ...
  document.querySelectorAll(".js-guide-shot[data-src]").forEach((placeholder) => {
    ...
    const img = new Image();
    img.onload = () => {
      img.alt = placeholder.getAttribute("data-title") || imageText.fallbackAlt;
      if (link) {
        ...
        anchor.setAttribute("aria-label", imageText.openLabel(img.alt));
        anchor.setAttribute("data-open-label", imageText.openPublicPage);
